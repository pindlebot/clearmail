...
const resolvers = {
  Query: {
    ...
    user: (obj, args, context) => {
      return User.createSession({ userId: context.user })
    }
  },
  Mutation: {
  ...
  Message: {
      ...
      return client.query(`
        ...
        join (
          select
            entity_id as id
          from
            messages_entities me
          where me.message_id = '${message.id}'
          group by me.entity_id
        ) x using (id)
        where e.entity_type = 'SOURCE'
      `).then(rows => rows.map(({ userId, ...rest }) => ({ ...rest, user: userId })))
    },
    destination: (message, _, { user }) => {
      return client.query(`
        select * from entities e
        join (
          select
    ...
    attachments: (message, _, { user }) => {
      return client.query(
        `SELECT * FROM attachments a WHERE a.message_id = uuid_or_null('${message.id}')`
      )
    },
